    ...
    * ``interrupt_check()`` — returns True to break the loop early.
    ...
    try:
        ...
        logger.debug(
            ...
            "recovering from collected events."
        )
...
def _sanitize_concrete_response_output(response: Any) -> Any:
    """Normalize concrete Responses objects that carry ``output=None``."""
...
def _mock_has_configured_behavior(fn: Any) -> bool:
    """Return True when a unittest.mock callable was explicitly configured."""
    try:
        from unittest.mock import DEFAULT, Mock

        return isinstance(fn, Mock) and (
            getattr(fn, "side_effect", None) is not None
            or getattr(fn, "_mock_return_value", DEFAULT) is not DEFAULT
        )
    except Exception:
        return False


def _should_use_legacy_sdk_stream(responses: Any) -> bool:
    """Prefer ``responses.stream`` for old clients or explicitly configured mocks."""
    create_fn = getattr(responses, "create", None)
    stream_fn = getattr(responses, "stream", None)
    if not callable(stream_fn):
        return False
    if not callable(create_fn):
        return True
    return _mock_has_configured_behavior(stream_fn) and not _mock_has_configured_behavior(create_fn)


def _consume_legacy_sdk_stream(
    agent,
    stream_obj: Any,
    *,
    model: str,
    ...
    """Consume the SDK ``responses.stream`` helper while preserving recovery."""
...
def run_codex_stream(agent, api_kwargs: dict, client: Any = None, on_first_delta=None):
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            if _should_use_legacy_sdk_stream(active_client.responses):
                with active_client.responses.stream(**api_kwargs) as legacy_stream:
                    return _consume_legacy_sdk_stream(
                        agent,
                        legacy_stream,
        ...
        try:
            ...
            if hasattr(event_stream, "output") and not hasattr(event_stream, "__iter__"):
                return _sanitize_concrete_response_output(event_stream)
