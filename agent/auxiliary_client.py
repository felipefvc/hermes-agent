    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            ...
            from agent.codex_runtime import (
                ...
                _sanitize_concrete_response_output,
                _should_use_legacy_sdk_stream,
            )
            ...
            if _should_use_legacy_sdk_stream(self._client.responses):
                with self._client.responses.stream(**resp_kwargs) as legacy_stream:
                    try:
                        ...
                        if "did not emit a terminal response" not in str(exc):
                            raise
                        get_final_response = getattr(legacy_stream, "get_final_response", None)
                        if not callable(get_final_response):
                            raise
                        final = get_final_response()
                    final = _sanitize_concrete_response_output(final)
            else:
                event_stream = self._client.responses.create(**stream_kwargs)
                try:
                    if hasattr(event_stream, "output") and not hasattr(event_stream, "__iter__"):
                        final = _sanitize_concrete_response_output(event_stream)
                    else:
                        final = _consume_codex_event_stream(
                            event_stream,
                            model=resp_kwargs.get("model"),
