    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                if "mention_names" in platform_cfg:
                    bridged["mention_names"] = platform_cfg["mention_names"]
                ...
                if plat == Platform.WHATSAPP and "observe_untriggered_group_messages" in platform_cfg:
                    bridged["observe_untriggered_group_messages"] = platform_cfg["observe_untriggered_group_messages"]
                if plat == Platform.WHATSAPP and "observe_unmentioned_group_messages" in platform_cfg:
                    bridged["observe_unmentioned_group_messages"] = platform_cfg["observe_unmentioned_group_messages"]
            ...
            if isinstance(whatsapp_cfg, dict):
                ...
                if "mention_names" in whatsapp_cfg and not os.getenv("WHATSAPP_MENTION_NAMES"):
                    os.environ["WHATSAPP_MENTION_NAMES"] = json.dumps(whatsapp_cfg["mention_names"])
                if (
                    "observe_untriggered_group_messages" in whatsapp_cfg
                    and not os.getenv("WHATSAPP_OBSERVE_UNTRIGGERED_GROUP_MESSAGES")
                ):
                    os.environ["WHATSAPP_OBSERVE_UNTRIGGERED_GROUP_MESSAGES"] = str(
                        whatsapp_cfg["observe_untriggered_group_messages"]
                    ).lower()
                if (
